function [data, axd, props] = data_pvelamp_corr( ftr, dimfun, data, formargs, param )
% peak velocity-amplitude correlation
%
% [data, axd, props] = DATA_PVELAMP_CORR( ftr, dimfun, data, formargs, param )
%
% INPUT
...
% props : axes properties (cell)
	...
	[axd, props] = axes_( param );
		...
		% proceed data
	logger.progress( 'determine peak velocity-amplitude correlation...' );
	...
	for di = 1:numel( data )
		...
		switch param
			case 'slope'
				corr(1) = util.corr( data{di}.vals(1, data{di}.fpos), data{di}.vals(2, data{di}.fpos), 1 )/pi;
				corr(2) = util.corr( data{di}.vals(1, ~data{di}.fpos), data{di}.vals(2, ~data{di}.fpos), 1 )/pi;
			case 'pearson'
				corr(1) = util.corr( data{di}.vals(1, data{di}.fpos), data{di}.vals(2, data{di}.fpos), 2 );
				corr(2) = util.corr( data{di}.vals(1, ~data{di}.fpos), data{di}.vals(2, ~data{di}.fpos), 2 );
			otherwise
...
function [axd, props] = axes_( param )
	...
	switch param
		case 'slope'
			axd(1).label = {'Stiffness c\surdk in \pi/s'};
			axd(1).ticks = [];
			axd(1).ticklabels = {};
